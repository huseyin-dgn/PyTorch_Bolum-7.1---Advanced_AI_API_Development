# ==========================================================
# 🧩 AŞAMA 10 — Model Entegrasyonlu API (Hazır /predict endpoint)
# ==========================================================
# - Dinamik route dekoratörü
# - Global Error Handler
# - Renkli + Dosyalı Loglama
# - Süre Ölçümü (X-Response-Time)
# - /predict endpoint → model.pkl veya model.pt üzerinden tahmin
# ==========================================================
...
@route("GET", "/")
def root():
    return "<h1>🧠 Model API Hazır</h1><p>/predict endpoint’ine JSON göndererek tahmin alabilirsin.</p>"
...
def predict(headers, body):
    ...
    if not X:
        raise ValueError("İstek içinde 'x' değeri bulunamadı.")

    model = None
    if os.path.exists("model.pkl"):
        log("model.pkl yüklendi (pickle)", CYAN, "📦")
        with open("model.pkl", "rb") as f:
            model = pickle.load(f)
        preds = model.predict(np.array(X).reshape(-1, 1)).tolist()

    elif os.path.exists("model.pt"):
        log("model.pt yüklendi (PyTorch)", CYAN, "🔥")
        model = torch.load("model.pt", map_location="cpu")
        model.eval()
        X_tensor = torch.tensor(X, dtype=torch.float32).unsqueeze(1)
        preds = model(X_tensor).detach().numpy().flatten().tolist()

    else:
        raise FileNotFoundError("Herhangi bir model dosyası (model.pkl / model.pt) bulunamadı.")
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
    ...
    srv.bind((HOST, PORT))
    srv.listen(64)
    log(f"Model API çalışıyor → http://{HOST}:{PORT}", CYAN, "🚀")
